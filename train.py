...
class Records:
    def __init__(self):
        self.train_losses, self.train_losses_wo_dropout, self.both_val_losses = [], [], []
        self.train_accs, self.train_accs_wo_dropout, self.both_val_accs = [], [], []
        self.train_custom_metrics, self.train_custom_metrics_wo_dropout, self.both_val_custom_metrics= [], [], []
        self.lrs = []
    ...
    def get_metrics(self):
        return ['train_accs_wo_dropout', 'both_val_accs', 'both_val_custom_metrics']
...
def validate(model, val_dl, criterion, records, data_name):
    ...
    if data_name == 'both':
        records.both_val_losses.append(val_loss / len(val_dl))
        records.both_val_accs.append(correct_count / total)
        records.both_val_custom_metrics.append(extra_score)
        print(f'\t both val loss={records.both_val_losses[-1]:.4f} | both val acc={records.both_val_accs[-1]:.4f} | '
              f'base val {extra_metric.__name__}={records.both_val_custom_metrics[-1]:.4f}')
...
def train(train_dl, val_both_dl, display_dl_iter, model, optimizer, n_epochs, max_lr, scheduler, criterion, train_source):
    records = Records()
    best_metric = 0.
    ...
    for epoch in range(n_epochs):
        ...
        validate(model, val_both_dl, criterion, records, data_name='both')
        ...
        display_predictions_on_image(model, val_both_dl.dataset.cached_path, display_dl_iter, name=display_filename)
