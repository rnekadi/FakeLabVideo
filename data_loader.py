...
def create_dataloaders(params):
    ...
    train_dl = _create_dataloader(f'/content/drive/My Drive/DeepFake-Detection/datasets/{params["train_data"]}_deepfake', mode='train',
                                  batch_size=params['batch_size'],
                                  transformations=train_transforms, sample_ratio=params['sample_ratio'])
    val_both_dl = _create_dataloader(f'/content/drive/My Drive/DeepFake-Detection/datasets/both_deepfake/val', mode='val', batch_size=params['batch_size'],
                                     transformations=val_transforms,
                                     sample_ratio=params['sample_ratio'])
    display_file_paths = [f'/content/drive/My Drive/DeepFake-Detection/datasets/{i}_deepfake/val' for i in ['both']]
    display_dl_iter = iter(
        _create_dataloader(display_file_paths, mode='val', batch_size=32, transformations=val_transforms,
                           sample_ratio=params['sample_ratio']))

    return train_dl, val_both_dl, display_dl_iter


def _create_dataloader(file_paths, mode, batch_size, transformations, sample_ratio, num_workers=0):
    ...
    ds = FFDataset(filenames, filepath=f'/content/drive/My Drive/DeepFake-Detection/datasets/precomputed/', transform=transformations, recompute=False)
